...
func trimGithubPrefix(fullpath string) string {
	i := strings.LastIndex(fullpath, "controllers")
	return fullpath[i:]
}

func isProjectControllersAndFixControllerPathPrefix(pkgPath string) (string, bool) {
	currentPath, _ := os.Getwd()
	pathSlice := strings.Split(currentPath, "/")
	projectName := pathSlice[len(pathSlice)-1]
	pkgPathSlice := strings.Split(pkgPath, "/")
	if len(pkgPathSlice) >= 2 && pkgPathSlice[1] == "controllers" {
		pkgPathSlice[0] = projectName
	}
	pkgPath = strings.Join(pkgPathSlice, "/")
	return pkgPath, strings.HasPrefix(pkgPath, projectName)
}

func analyseControllerPkg(vendorPath, localName, pkgpath string) {
	pkgpath = strings.Trim(pkgpath, "\"")
	var ok bool
	if pkgpath, ok = isProjectControllersAndFixControllerPathPrefix(pkgpath); !ok {
		return
	}
